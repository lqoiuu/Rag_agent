...
- 修法（代码驱动，非再调一次模型）：在 `classify` 之后加一条改判规则，命中时把意图改为 `device`，并在轨迹中留痕 `classify:device:rerouted-from-knowledge`，使改判永远不会被误认为模型自己的判断。四个条件是：意图可被替换（`ticket` 永不改判）、设备号已可从消息或窗口解析、**消息是指代式追问或询问只存在于设备存储的字段**、消息不含产品知识话题词。
- 后续补正（提交 `4f14fb9`）：最初把第三个条件写成"必须是指代式追问"，结果**明确点名设备号**的提问（如「D2002 还在保修吗」）不会被改判，在运行中的界面里真实出现过「模型判成 knowledge → 答拒」的现象。现改为「指代线索 **或** 设备记录类词（保修/订单/购买/激活/到期/序列号）」，同时保留知识话题词的否决权。另加一条测试断言两张词表**互不相交**——因为「保修」与时长词曾两次被误列进知识话题词，使这条规则对它要修的问题失效。
- 真实模型测量：`D2002 还在保修吗` 在空会话 3/3、暖会话 3/3 都判为 `device`；截图中那条会话的检查点记录也全部是 `device` 且正常作答。因此该误判是**偶发而非稳定复现**，这正是用确定性规则兜底而不是继续调提示词的理由。
