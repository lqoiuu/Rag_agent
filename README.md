...
当前状态：阶段 7（RAG 回答与来源引用）已完成并通过验收。
...
**已知检索限制**：真实实测显示，绝对余弦相似度阈值无法区分「资料能回答」与「资料无法回答」——正确答案与无关问题的最高分几乎相同（0.5889 与 0.5897）。因此 `RAG_AGENT_RETRIEVAL_THRESHOLD` 只是很松的下界，**只用于避免无意义的模型调用，不承担拒答职责**；拒答由「只能依据资料回答」的约束与引用校验承担，并在评测阶段用数据校准。同理，目录类分片已用确定性规则在入库时过滤，其效果（前两名分差由 0.0011 提升到 0.0375）已在项目结构文档中记录，后续由评测集复核。
