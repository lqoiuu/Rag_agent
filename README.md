...
当前状态：阶段 6（检索器 V1 与可解释结果）已完成并通过验收。
...
- 依据检索结果生成带引用的答案，以及答案侧的拒答。
- LangGraph Agent 工作流。
- Streamlit 页面。
- 矢量轮廓（文字转曲线）PDF 的文本提取，需要 OCR，当前明确不支持。

**已知检索限制**：真实实测显示，绝对余弦相似度阈值无法区分「资料能回答」与「资料无法回答」——正确答案与无关问题的最高分几乎相同（0.5889 与 0.5897）。因此 `RAG_AGENT_RETRIEVAL_THRESHOLD` 只是很松的下界，**不能作为拒答机制**；拒答将依赖后续阶段的依据约束与引用校验，并在评测阶段用数据校准。
